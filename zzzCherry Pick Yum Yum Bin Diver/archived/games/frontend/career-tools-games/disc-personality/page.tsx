...
interface QuestionResponse {
  ...
  questionType: 'core' | 'personalized';
  context: string;
  title: string;
  scenario: string;
  options: Array<{
...
export default function FilipinoDiscGame() {
  ...
  const QUESTION_BANK_VERSION = 'filipino_disc_v1_2026_01_21';
  ...
  const startGame = () => {
    ...
    sessionIdRef.current = (typeof crypto !== 'undefined' && 'randomUUID' in crypto)
      ...
      : `disc-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`;
  ...
  const generatePersonalizedQuestions = async (responses?: any[], scores?: any) => {
    // Skip personalized questions for anonymous users
    if (!user) {
      console.log('⚠️ Skipping personalized questions for anonymous user');
      completeGame();
      return;
    }
    ...
    console.log('🤖 Starting personalized questions generation for user:', user?.id || 'anonymous');
    setGameState(prev => ({ ...prev, isGeneratingPersonalized: true }));
    
    try {
      console.log('🤖 Generating personalized questions for', user?.email || 'anonymous');
...
${allResponses.map(r => `Q${r.questionId} [${r.context || 'UNKNOWN'}] ${r.title || ''}\nScenario: ${r.scenario || ''}\nSelected: "${r.selectedOptionText || ''}" (Choice ${r.selectedChoice}, ${r.discType})\nReaction: "${r.selectedOptionReaction || ''}"\nResponse time: ${r.responseTime}ms`).join('\n\n')}
...
Make it deeply personal and actionable based on their actual choices.`;
  ...
  const handleOptionSelect = (optionId: string, disc: string, reaction: string) => {
    ...
    // Record response
    const isPersonalized = gameState.showPersonalized && gameState.currentQuestion >= FILIPINO_DISC_SCENARIOS.length;
    const scenario = isPersonalized
      ? gameState.personalizedQuestions[gameState.currentQuestion - FILIPINO_DISC_SCENARIOS.length]
      : FILIPINO_DISC_SCENARIOS[gameState.currentQuestion];
    const selectedOption = scenario?.options?.find((opt: any) => opt.id === optionId);

    const response: QuestionResponse = {
      questionId: scenario?.id || gameState.currentQuestion + 1,
      questionIndex: gameState.currentQuestion,
      questionType: isPersonalized ? 'personalized' : 'core',
      context: scenario?.context || 'UNKNOWN',
      title: scenario?.title || '',
      scenario: scenario?.scenario || '',
      options: Array.isArray(scenario?.options) ? scenario.options : [],
      selectedChoice: optionId,
      selectedOptionText: selectedOption?.text || '',
      selectedOptionReaction: selectedOption?.reaction || reaction || '',
      discType: disc,
      responseTime: startTime - (lastQuestionStartRef.current || startTime),
      timestamp: new Date().toISOString(),
      questionBankVersion: QUESTION_BANK_VERSION
    };

    saveAnswerEvent(response).catch(() => {});
  ...
  const saveAnswerEvent = async (response: QuestionResponse) => {
    if (!user) return;
    const sessionId = sessionIdRef.current || `disc-${Date.now()}-${Math.random().toString(36).substring(2, 10)}`;
    sessionIdRef.current = sessionId;
    try {
      await fetch('/api/games/disc/answer', {
        method: 'POST',
        headers: { 
          'Content-Type': 'application/json',
  ...
  const saveSession = async (results: any, aiAssessment: string | null, aiBpoRoles: any[] | null) => {
    if (!user) return;
    try {
  ...
  const updateSessionAI = async (aiAssessment: string | null, aiBpoRoles: any[] | null) => {
    if (!user) return;
  ...
  const calculateResults = async (scores: { D: number; I: number; S: number; C: number }) => {
    ...
    const calculateCulturalAlignment = () => {
      ...
      const contextScores = culturalContexts.map(context => {
        const contextResponses = gameState.responses.filter((r, index) => {
          if (r.context && r.context !== 'UNKNOWN') return r.context === context;
    ...
    if (backgroundMusic) {
      ...
      saveSession(results, null, null);

      // Generate AI in background and patch when ready
      Promise.allSettled([
        generateAIAssessment(results, gameState.responses),
        generateAIBpoRoles(results, gameState.responses)
      ]).then(([assessmentResult, rolesResult]) => {
        const generatedAssessment = assessmentResult.status === 'fulfilled' ? assessmentResult.value : null;
        const generatedBpoRoles = rolesResult.status === 'fulfilled' ? rolesResult.value : null;
        updateSessionAI(generatedAssessment, generatedBpoRoles);
      });
    }, 5000);
  };
