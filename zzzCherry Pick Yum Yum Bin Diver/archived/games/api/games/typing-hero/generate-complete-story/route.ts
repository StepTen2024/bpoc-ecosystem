...
import { type StoryChapter } from '@/lib/story-storage';
...
// Helper to create complete story object
function createCompleteStory(
  ...
  const storyId = `story_${userId.substring(0, 8)}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
  
  return {
    id: storyId,
    userId,
    title: `${userProfile.name}'s Typing Journey`,
    difficulty,
    chapters,
...
export async function POST(request: NextRequest) {
  let safeUserProfile: UserProfile | null = null;
  let safeGameData: GameProgressData | null = null;
  ...
  try {
    ...
    safeUserProfile = userProfile || null;
    safeGameData = gameData || null;
    ...
    if (!anthropicApiKey) {
      console.warn('⚠️ Missing ANTHROPIC_API_KEY or CLAUDE_API_KEY, using fallback story');
      const fallbackChapters = generateFallbackChapters(userProfile, gameData);
      const fallbackStory = createCompleteStory(
        userProfile.userId,
        userProfile,
        gameData.difficulty,
        fallbackChapters
    ...
    // Call Claude API for complete story using SDK
    ...
    try {
      ...
      let storyText = '';
      if (aiResponse.content && aiResponse.content[0] && 'text' in aiResponse.content[0]) {
        storyText = aiResponse.content[0].text;
      } else {
        throw new Error('Invalid Claude API response format');
      }

      // Process the complete story response
      let chapters = processCompleteStoryResponse(storyText, gameData);
      if (!chapters || chapters.length !== 5) {
        console.warn('⚠️ Claude response missing 5 chapters, using fallback story');
        chapters = generateFallbackChapters(userProfile, gameData);
      }
      ...
      console.warn('⚠️ Claude API error, using fallback story:', errorMessage);
      const fallbackChapters = generateFallbackChapters(userProfile, gameData);
      const fallbackStory = createCompleteStory(
        userProfile.userId,
        userProfile,
    ...
    console.warn('⚠️ Story generation failed, using fallback story');
    const fallbackProfile = safeUserProfile || { userId: 'unknown', name: 'Professional' };
    const fallbackGameData = safeGameData || { difficulty: 'rockstar' };
    const fallbackChapters = generateFallbackChapters(fallbackProfile, fallbackGameData);
    const fallbackStory = createCompleteStory(
      fallbackProfile.userId,
      fallbackProfile,
      fallbackGameData.difficulty,
      fallbackChapters
    );
    return NextResponse.json(fallbackStory);
  }
...
function generateFallbackChapters(userProfile: UserProfile, gameData: GameProgressData): StoryChapter[] {
  const firstName = (userProfile?.name || 'Professional').split(' ')[0] || 'Professional';
  const difficulty = (gameData?.difficulty || 'rockstar').toLowerCase();

  const chapterDefs = [
    { number: 1, title: 'Starting Fresh', theme: 'Learning the basics', difficulty: 'easy', words: 6 },
    { number: 2, title: 'Building Skills', theme: 'Taking action', difficulty: 'easy', words: 7 },
    { number: 3, title: 'Growing Confidence', theme: 'Solving real tasks', difficulty: 'medium', words: 8 },
    { number: 4, title: 'Mastering the Flow', theme: 'Handling pressure', difficulty: 'medium', words: 9 },
    { number: 5, title: 'Future Ready', theme: 'Leading with focus', difficulty: 'hard', words: 10 },
  ];

  const makeSentence = (id: number, text: string, level: string) => ({
    id,
    text,
    words: text.split(' '),
    difficulty: level,
  ...
  return chapterDefs.map((chapter) => {
    const sentences = [
      `${firstName} starts a new shift with steady focus`,
      `Each message builds confidence in the role`,
      `Clear communication helps every customer feel heard`,
      `Team support keeps the work moving forward`,
      `Small wins turn into strong momentum today`,
      `Professional growth comes from consistent practice`,
    ];

    const trimmedSentences = sentences
      .map((sentence) => sentence.split(' ').slice(0, chapter.words).join(' '))
      .slice(0, 5);
    ...
    return {
      ...
      estimatedDuration: 90 + chapter.number * 15,
      sentences: trimmedSentences.map((text, index) => makeSentence(index + 1, text, chapter.difficulty)),
    };
  });
}
