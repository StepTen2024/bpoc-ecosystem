...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!research) {
      console.warn('⚠️ No research data provided to Stage 3. This may lead to poor plan quality.');
      // We don't block it, but we log it.
    }

    // Generate comprehensive plan with Claude Sonnet 4
    // Added explicit timeout handling for the fetch process if needed in the future,
    // but relying on maxDuration=180 for now.
    ...
    if (insightId) {
      ...
      const result = await supabase
        .from('insights_posts')
        .insert({
          slug: plan.structure?.slug || topic.toLowerCase().replace(/[^a-z0-9]+/g, '-'),
          title: plan.structure?.title || plan.title || topic,
          content: '',
          meta_description: plan.structure?.metaDescription || plan.metaDescription,
          author: 'Admin',
...
IMPORTANT: Return ONLY valid JSON, no markdown code blocks or explanations.`
    ...
    if (apiError.status === 401) throw new Error('Claude API Key Invalid');
    if (apiError.status === 402) throw new Error('Claude API Credits Exhausted');
    if (apiError.status === 429) throw new Error('Claude API Rate Limit Exceeded during plan generation');
    if (apiError.status === 500) throw new Error('Claude API Service Error');
    if (apiError.status === 529) throw new Error('Claude API Overloaded - Please try again in 30s');
