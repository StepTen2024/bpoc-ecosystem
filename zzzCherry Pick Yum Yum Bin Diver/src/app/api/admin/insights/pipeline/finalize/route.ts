...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const research = pipeline.research_results;
    const plan = pipeline.article_plan;
    const article = pipeline.optimized_article || pipeline.humanized_article || pipeline.raw_article;
    const title = plan?.title || pipeline.title;
    const keywords = plan?.keywords?.main || [];
    const seoOptimization = pipeline.seo_optimization;
    const metaGeneration = pipeline.meta_generation;
    const mediaGeneration = pipeline.media_generation;
    ...
    const slug = metaGeneration?.meta?.canonicalSlug || 
    ...
    const articleData = {
      ...
      // Media
      hero_type: mediaGeneration?.video ? 'video' : 'image',
      hero_url: mediaGeneration?.video?.url || mediaGeneration?.images?.[0]?.url,
      ...
      category: plan?.silo || pipeline.category || 'BPO & Outsourcing',
      silo_topic: plan?.silo,
...
async function runQualityChecks(
  ...
  const keywordMatches = primaryKeyword 
    ...
    : 0;
  const keywordDensity = keywordMatches / wordCount;
