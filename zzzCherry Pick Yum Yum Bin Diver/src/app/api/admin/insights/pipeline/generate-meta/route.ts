...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // For silo pages: override with silo editor values (source of truth)
    if (isSilo && siloMeta) {
      if (siloMeta.seo_title) {
        console.log(`📌 Overriding metaTitle with silo editor: "${siloMeta.seo_title}"`);
        metaTags.metaTitle = siloMeta.seo_title;
      }
      if (siloMeta.seo_description) {
        console.log(`📌 Overriding metaDescription with silo editor: "${siloMeta.seo_description.slice(0, 60)}..."`);
        metaTags.metaDescription = siloMeta.seo_description;
      }
    }
    ...
    // ============================================
    console.log('\n📄 Step 5: Generating schema.org markup...');
    const schemaMarkup = await generateSchemaMarkup(
      ...
      safeKeywords,
      author,
      category || plan?.silo || 'BPO & Outsourcing'
    );
    console.log('✅ Schema markup generated');
    console.log(`   - Types: ${Object.keys(schemaMarkup).join(', ')}`);
...
async function generateSchemaMarkup(
  ...
  const faqMatch = article.match(/##\s*(?:FAQ|Frequently Asked Questions)([\s\S]*?)(?=##|$)/i);
  const faqItems = faqMatch ? extractFAQItems(faqMatch[1]) : [];

  // Extract How-To steps if exists
  const howToMatch = article.match(/##\s*(?:How to|Steps)([\s\S]*?)(?=##|$)/i);
  const howToSteps = howToMatch ? extractHowToSteps(howToMatch[1]) : [];
  ...
  const schema: SchemaMarkup = {
    ...
    organization: {
      ...
      description: 'Your trusted partner for BPO career success and outsourcing solutions',
      sameAs: [
        'https://www.linkedin.com/company/bpoc-careers',
        'https://www.facebook.com/BPOCCareers',
    ...
    breadcrumb: {
      ...
      itemListElement: [
        ...
        },
        {
          ...
          position: 3,
          name: category,
          item: `${baseUrl}/insights?category=${encodeURIComponent(category)}`,
        },
        {
          '@type': 'ListItem',
          position: 4,
          name: title,
          item: `${baseUrl}/insights/${slug}`,
        },
      ],
    },
  };
 ...
 */
function extractFAQItems(faqSection: string): Array<{ question: string; answer: string }> {
  const items: Array<{ question: string; answer: string }> = [];
  
  // Match patterns like "**Q: Question?**" or "### Question?"
  const regex = /(?:\*\*Q:\s*(.+?)\*\*|###\s+(.+?))\s*\n\s*(?:\*\*A:\s*(.+?)(?=\*\*Q:|###|$)|(.+?)(?=\*\*Q:|###|$))/g;
  
  let match;
  while ((match = regex.exec(faqSection)) && items.length < 10) {
    const question = match[1] || match[2];
    const answer = match[3] || match[4];
 ...
 */
function extractHowToSteps(howToSection: string): Array<{ name: string; text: string }> {
  const steps: Array<{ name: string; text: string }> = [];
  
  // Match numbered lists: "1. Step name" or "Step 1: Name"
  const regex = /(?:^|\n)\s*(?:(\d+)\.|\*\*Step\s+(\d+):?\*\*)\s*([^\n]+)\n\s*([^\n]+(?:\n(?!\d+\.|Step\s+\d+).+)*)/gm;
  
  let match;
  while ((match = regex.exec(howToSection)) && steps.length < 20) {
    ...
    if (stepName) {
      steps.push({
        name: stepName.trim(),
        text: stepText ? stepText.trim().replace(/\n+/g, ' ') : stepName.trim(),
      });
    }
  }
