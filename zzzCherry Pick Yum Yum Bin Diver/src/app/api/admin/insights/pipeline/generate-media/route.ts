...
2. SEO alt text (descriptive, include primary keyword: "${keywords[0]}")
...
async function uploadImageToSupabase(image: ImageResult, articleTitle: string): Promise<string> {
  const slug = articleTitle.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '');
  const sectionSlug = image.section?.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || 'image';
  const filename = `images/${slug}-${sectionSlug}.png`;
