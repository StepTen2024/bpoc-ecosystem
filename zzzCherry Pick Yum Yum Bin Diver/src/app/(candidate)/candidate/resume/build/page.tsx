...
export default function ResumeBuildPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        // Try API first if logged in (database is source of truth)
        if (user?.id) {
          ...
          if (sessionToken) {
            ...
            if (res.ok) {
              ...
              // Debug: Log the full API response for avatar
              console.log('🔍 Resume API response:', {
                hasData: data.hasData,
                profilePhoto: data.profilePhoto,
                profileAvatarUrl: data.profile?.avatar_url,
                profile: data.profile,
              });

              // Always load profile photo from candidate avatar_url if available
              const candidateAvatarUrl = data.profilePhoto || data.profile?.avatar_url;
              if (candidateAvatarUrl) {
                setProfileImage(candidateAvatarUrl);
                console.log('📸 Candidate avatar loaded:', candidateAvatarUrl);
              } else {
                console.log('⚠️ No avatar found in API response, will try direct fetch');
              }

              // Store candidate profile if available
              if (data.profile) {
                setCandidateProfile(data.profile);
              }

              // Prioritize improvedResume (which includes generated_data), then fallback to others
              const resumePayload = data.improvedResume || data.resumeData || data.extractedResume || data.resume?.resume_data;
              if (data.hasData && resumePayload) {
                const normalized = normalizeResumeData(resumePayload);
                
                // Auto-populate phone, location from candidate profile if missing in resume
                if (data.profile) {
                  if ((!normalized.phone || normalized.phone === 'Add phone' || normalized.phone === '+63 912 345 6789') && data.profile.phone) {
                    normalized.phone = data.profile.phone;
                  }
                  if ((!normalized.location || normalized.location === 'Add location' || normalized.location === 'Manila, Philippines') && data.profile.location) {
                    normalized.location = data.profile.location;
                  }
                }
                
                setResumeData(normalized);

                // Override with resume-specific photo if available
                const resumePhotoUrl = resumePayload.profilePhoto || resumePayload.photoUrl;
                if (resumePhotoUrl) {
                  setProfileImage(resumePhotoUrl);
                  console.log('📸 Resume photo loaded:', resumePhotoUrl.substring(0, 50) + '...');
                }

                if (data.template?.id && layoutStyles[data.template.id]) {
                  setSelectedTemplate(data.template.id);
                }
                // Load saved colors from API if available
                ...
                // Also load template from resumePayload if available
                if (resumePayload?.selectedTemplate) setSelectedTemplate(resumePayload.selectedTemplate);
                // Save to localStorage to keep in sync
                localStorage.setItem('bpoc_generated_resume', JSON.stringify(normalized));

                // If still no profile image, fetch directly from candidates table
                if (!candidateAvatarUrl && user?.id) {
                  console.log('📸 No avatar from resume API, fetching from candidates...');
                  try {
                    const candidateRes = await fetch(`/api/candidates/${user.id}`);
                    if (candidateRes.ok) {
                      const candidateData = await candidateRes.json();
                      console.log('📸 Candidates API response:', { avatar_url: candidateData.candidate?.avatar_url });
                      if (candidateData.candidate?.avatar_url) {
                        setProfileImage(candidateData.candidate.avatar_url);
                        console.log('📸 Avatar loaded from candidates table:', candidateData.candidate.avatar_url);
                      }
                    }
                  } catch (err) {
                    console.warn('Failed to fetch candidate avatar:', err);
                  }
                }
              ...
              // Even if no resume data, try to fetch avatar directly
              if (user?.id) {
                console.log('📸 No resume data, fetching avatar from candidates...');
                try {
                  const candidateRes = await fetch(`/api/candidates/${user.id}`);
                  if (candidateRes.ok) {
                    const candidateData = await candidateRes.json();
                    if (candidateData.candidate?.avatar_url) {
                      setProfileImage(candidateData.candidate.avatar_url);
                      console.log('📸 Avatar loaded from candidates:', candidateData.candidate.avatar_url);
                    }
                  }
                } catch (err) {
                  console.warn('Failed to fetch candidate avatar:', err);
        ...
        // Fallback to localStorage if API didn't return data (for guests or if no saved resume)
        ...
        if (stored) {
          ...
          if (parsed?.profilePhoto || parsed?.photoUrl) {
            setProfileImage(parsed.profilePhoto || parsed.photoUrl);
          }
          ...
          if (parsed?.colorScheme) setSelectedColorScheme(parsed.colorScheme);

          // If no profile photo yet but user is logged in, try to fetch avatar_url
          if (!parsed?.profilePhoto && !parsed?.photoUrl && user?.id) {
            try {
              const candidateRes = await fetch(`/api/candidates/${user.id}`);
              if (candidateRes.ok) {
                const candidateData = await candidateRes.json();
                if (candidateData.candidate?.avatar_url) {
                  setProfileImage(candidateData.candidate.avatar_url);
                  console.log('📸 Avatar loaded from candidate profile:', candidateData.candidate.avatar_url.substring(0, 50) + '...');
                }
              }
            } catch (avatarErr) {
        ...
        setResumeData(normalizeResumeData(MOCK_RESUME));

        // Still try to fetch candidate avatar even with mock data
        if (user?.id) {
          try {
            console.log('📸 Fetching candidate avatar for mock data view...');
            const candidateRes = await fetch(`/api/candidates/${user.id}`);
            if (candidateRes.ok) {
              const candidateData = await candidateRes.json();
              if (candidateData.candidate?.avatar_url) {
                setProfileImage(candidateData.candidate.avatar_url);
                console.log('📸 Avatar loaded from candidate:', candidateData.candidate.avatar_url.substring(0, 50) + '...');
              }
            }
          } catch (avatarErr) {
            console.warn('Could not fetch avatar for mock view:', avatarErr);
          }
        }

        toast.info('Using demo data - Upload a resume to see your own!');
        setLoading(false);
