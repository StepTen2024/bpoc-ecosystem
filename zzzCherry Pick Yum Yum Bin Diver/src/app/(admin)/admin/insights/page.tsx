...
import { Plus, Edit, Trash2, Eye, FileText, BarChart2, Activity, Search, Zap, Loader2, CheckCircle, XCircle, TrendingUp, AlertTriangle, X, AlertOctagon, FolderTree, Crown, Newspaper } from 'lucide-react';
...
import ArticleGenerator from '@/components/admin/insights/ArticleGenerator';
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="list" className="w-full">
        <TabsList className="bg-white/5 border border-white/10 mb-6">
          ...
          <TabsTrigger value="generate" className="data-[state=active]:bg-orange-500/20 data-[state=active]:text-orange-400">
        ...
        <TabsContent value="list">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-slate-800/80 to-slate-900/80 border border-slate-700/50 p-5">
              <div className="absolute top-0 right-0 w-20 h-20 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2" />
              <p className="text-slate-400 text-xs font-medium tracking-wider uppercase mb-1">Total</p>
              <p className="text-3xl font-bold text-white">{posts.length}</p>
            </div>
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-950/80 to-emerald-900/40 border border-emerald-800/50 p-5">
              <div className="absolute top-0 right-0 w-20 h-20 bg-emerald-500/10 rounded-full -translate-y-1/2 translate-x-1/2" />
              <p className="text-emerald-400/80 text-xs font-medium tracking-wider uppercase mb-1">Live</p>
              <p className="text-3xl font-bold text-emerald-400">{posts.filter(p => p.is_published).length}</p>
            </div>
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-950/80 to-amber-900/40 border border-amber-800/50 p-5">
            ...
            <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-cyan-950/80 to-cyan-900/40 border border-cyan-800/50 p-5">
              <div className="absolute top-0 right-0 w-20 h-20 bg-cyan-500/10 rounded-full -translate-y-1/2 translate-x-1/2" />
              <p className="text-cyan-400/80 text-xs font-medium tracking-wider uppercase mb-1">Avg SEO</p>
              {(() => {
                const avgScore = posts.length > 0 ? Math.round(posts.reduce((acc, p) => acc + calculateSEOScore(p), 0) / posts.length) : 0;
                const style = getScoreStyle(avgScore);
                return <p className={`text-3xl font-bold ${style.color}`}>{avgScore}<span className="text-lg opacity-60">/100</span></p>;
              })()}
            </div>
          </div>
              ...
              {posts.filter(p => p.is_published).length > 0 && (
                <section>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-emerald-400/60 text-sm">{posts.filter(p => p.is_published).length} articles live</span>
                  </div>
                  ...
                  <div className="flex gap-2 mb-4">
                    ...
                    >
                      ...
                      <Badge variant="outline" className={`text-[10px] ml-1 ${publishedTab === 'silo' ? 'border-violet-500/30 text-violet-400' : 'border-slate-600 text-slate-500'}`}>
                        {posts.filter(p => p.is_published && p.is_pillar === true).length}
                      </Badge>
                    </button>
                    <button
                  ...
                  <div className="grid gap-3">
                    ...
                    {posts.filter(p => p.is_published && (publishedTab === 'silo' ? p.is_pillar === true : p.is_pillar !== true)).length === 0 && (
              ...
              {posts.filter(p => !p.is_published).length > 0 && (
                <section>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-amber-400/60 text-sm">{posts.filter(p => !p.is_published).length} articles</span>
                  </div>
                  ...
                  <div className="grid gap-3">
                    ...
                    {posts.filter(p => !p.is_published && (draftsTab === 'silo' ? p.is_pillar === true : p.is_pillar !== true)).length === 0 && (
                      <div className="text-center py-10 text-slate-500">
                        <FileText className="w-12 h-12 mx-auto mb-3 opacity-30" />
